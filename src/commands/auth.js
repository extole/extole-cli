...
Examples:
  ...
  if (defaultCmd) defaultCmd._mcpDescription = 'Set the default account used when no --account flag is passed. Call this only when the user explicitly says "switch to account X" or "make X my default" — not as a convenience when running a one-off command against a different account. For one-off calls use --account per-command instead, which avoids silently changing the default for all subsequent calls.';
  if (tokenCmd) tokenCmd._mcpDescription = 'Print the raw bearer token for a saved account. SECURITY SENSITIVE — output is a live credential that must not be logged or shared. Call this only when the user explicitly asks for the token value, or when piping into a tool that requires a raw Bearer token (e.g. a curl command the user is constructing). Do not call this to check if a token is valid — use whoami for that.';
